...
vector<string> decide_winner(const vector<vector<int>>& point_values)
{
    vector<string> winner_info;
    ...
    if(results.empty())
    {
        winner_info.push_back("No players");
        return winner_info;
    }
    ...
    {
        // outputstream << iter->second.at(0) ;
       string winner = std::to_string(iter->second.at(0));
       winner_info.push_back(winner);
    }
    else
    {   // << "Result is tie between players "
        string winner = std::to_string(iter->second.at(0));
        winner_info.push_back(winner);

        for(unsigned int i = 1; i < iter->second.size(); ++i)
        {
            string tied = std::to_string(iter->second.at(i));
            winner_info.push_back(tied);
        }
    }

    winner_info.push_back(RESULT_TEXTS.at(static_cast<Result>(iter->first)));
